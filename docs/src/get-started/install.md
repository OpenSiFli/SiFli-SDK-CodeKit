...
![](images/image_install_2.png)

2. 点击`下载新版本`按钮

![](images/image_install_3.png)

如果你在`中国大陆`地区，SDK 来源建议使用`Gitee`；工具链下载源建议选择`SiFli 国内镜像`，以获得更快的下载速度。

我们有`发布版本(Release/Tag)`和`开发分支(Branch)`两种安装选项：

- `发布版本(Release/Tag)`：适合大多数用户，经过充分测试，稳定可靠。其版本号通常为 `vX.Y.Z` 格式（例如 `v2.4.4`）。其本质是 SiFli-SDK git仓库的一个特定标签（tag）。
- `开发分支(Branch)`：适合需要最新功能和修复的用户，但可能不够稳定。其本质是一个git 分支（branch），通常为 `main` 或 `release/v2.4`等。
...
![](images/image_install_4.png)
3. 选择安装版本和安装路径后，点击`开始安装`按钮，等待安装完成。

在安装过程中，会持续更新目前的安装日志，请耐心等待。这个过程中会使用git克隆SiFli-SDK仓库，并下载所需的工具链文件，可能需要一些时间，具体取决于你的网络速度。

4. 如果一切顺利，安装完成后，你会看到如下提示：

![](images/image_install_5.png)

点击`返回`按钮，安装过程即告结束。你现在可以开始使用 SiFli-SDK-CodeKit 进行项目开发了！
![](images/image_install_6.png)