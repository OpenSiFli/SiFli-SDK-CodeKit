...
export class LanguageModelToolService {
    ...
    const tool: vscode.LanguageModelTool<T> = {
      prepareInvocation: async options => {
        ...
        return {
          ...
          confirmationMessages: descriptor.confirmationTitle || descriptor.confirmationMessage
            ? {
  ...
  private listWorkflows(scope: 'workspace' | 'user' | 'all'): unknown[] {
    const workflows = this.workflowService.getAllScopedWorkflows()
  ...
  private async compile(): Promise<unknown> {
    ...
    return this.buildOperationPayload('compile', runId, result.success, result.exitCode, result.command, result.message);
  }
  ...
  private async rebuild(): Promise<unknown> {
    ...
    return this.buildOperationPayload('rebuild', runId, compileResult.success, compileResult.exitCode, compileResult.command, compileResult.message);
  }
  ...
  private async download(): Promise<unknown> {
    ...
    return this.buildOperationPayload('download', runId, result.success, result.exitCode, result.command, result.message, {
