<template>
  <div class="min-h-screen bg-vscode-background text-vscode-foreground font-vscode relative overflow-hidden">
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div
        class="absolute top-0 left-0 w-96 h-96 bg-gradient-to-br from-red-500/10 to-orange-500/10 rounded-full blur-3xl transform -translate-x-1/2 -translate-y-1/2"
      ></div>
      <div
        class="absolute bottom-0 right-0 w-96 h-96 bg-gradient-to-tl from-gray-500/10 to-purple-500/10 rounded-full blur-3xl transform translate-x-1/2 translate-y-1/2"
      ></div>
    </div>
    ...
    <div class="relative z-10 p-8">
      <div class="max-w-4xl mx-auto">
        ...
        <div class="vscode-card rounded-lg p-6 border border-red-500/30 bg-vscode-editor-background/60">
          ...
          >
            <div v-if="logs.length === 0" class="text-gray-400 italic">暂无日志输出</div>
            <div v-else class="space-y-1">
              <div v-for="(log, index) in logs" :key="index" class="leading-tight">
