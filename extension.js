...
let compileBtn,
  ...
  downloadBtn,
  menuconfigBtn,
  currentBoardStatusItem,
  sdkManageBtn,
  currentSerialPortStatusItem,
  currentSdkVersionStatusItem;
...
const SIFLI_SDK_GITHUB_REPO_BASE =
  "https://api.github.com/repos/OpenSiFli/SiFli-SDK";
const SIFLI_SDK_GITEE_REPO_BASE =
  "https://gitee.com/api/v5/repos/SiFli/sifli-sdk";
...
async function getCompileCommand(boardName, threads) {
  ...
  const currentBoardDetails = availableBoardsDetails.find(
    (b) => b.name === boardName,
  );
  ...
  if (currentBoardDetails) {
    if (currentBoardDetails.type === "sdk") {
      ...
      boardSearchArg = "";
      console.log(
        `[SiFli Extension] Board '${boardName}' is from SDK. No --board_search_path needed.`,
      );
      ...
      const relativeToProject = path.relative(
        projectPath,
        projectLocalBoardsDir,
      );
      boardSearchArg = `--board_search_path="${relativeToProject.replace(/\\/g, "/")}"`;
      console.log(
        `[SiFli Extension] Board '${boardName}' is from project local. Using --board_search_path="${relativeToProject}".`,
      );
      ...
      const isSameDrive =
        path.parse(customBoardSearchDir).root.toLowerCase() ===
        path.parse(projectPath).root.toLowerCase();

      if (isSameDrive) {
        // 如果在同一盘符,计算相对路径
        const relativeToProject = path.relative(
          projectPath,
          customBoardSearchDir,
        );
        boardSearchArg = `--board_search_path="${relativeToProject.replace(/\\/g, "/")}"`;
        console.log(
          `[SiFli Extension] Board '${boardName}' is from custom path on same drive. Using --board_search_path="${relativeToProject}".`,
        );
      } else {
        // 如果不在同一盘符,使用绝对路径
        boardSearchArg = `--board_search_path="${customBoardSearchDir.replace(/\\/g, "/")}"`; // SCons 通常接受正斜杠的绝对路径
        console.log(
          `[SiFli Extension] Board '${boardName}' is from custom path on different drive. Using absolute --board_search_path="${customBoardSearchDir}".`,
        );
      }
    }
  } else {
    ...
    vscode.window.showWarningMessage(
      `当前选择的芯片模组 "${boardName}" 未找到有效配置。请在设置中重新选择。`,
    );
    console.warn(
      `[SiFli Extension] Selected board "${boardName}" not found in discovered boards.`,
    );
 ...
 */
async function getMenuconfigCommand(boardName) {
  // 变为 async 函数
  const config = vscode.workspace.getConfiguration("sifli-sdk-codekit");
  const workspaceFolders = vscode.workspace.workspaceFolders;
  const workspaceRoot =
...
async function readSftoolParamJson(boardName) {
  ...
  console.log(
    `[SiFli Extension] Reading sftool_param.json from: ${jsonFilePath}`,
  );
  ...
  try {
    ...
    if (
      !sftoolParam.chip ||
      !sftoolParam.write_flash ||
      !Array.isArray(sftoolParam.write_flash.files)
    ) {
      vscode.window.showErrorMessage(
        `sftool_param.json 结构无效。缺少 'chip' 或 'write_flash.files' 字段。`,
      );
      console.error(
        `[SiFli Extension] Invalid sftool_param.json structure:`,
        sftoolParam,
      );
      return null;
    }
    ...
    console.log(`[SiFli Extension] Parsed sftool_param.json:`, {
    ...
    vscode.window.showErrorMessage(
      `解析 sftool_param.json 失败: ${error.message}。请检查文件内容是否为有效 JSON。`,
    );
    console.error(
      `[SiFli Extension] Failed to read or parse sftool_param.json:`,
      error,
    );
    return null;
  }
...
async function getSftoolDownloadCommand(boardName, serialPortNum) {
  if (!boardName) {
    vscode.window.showWarningMessage(
      "未选择 SiFli 芯片模组。请点击状态栏中的“SiFli Board: N/A”选择模组。",
    );
    return "";
  }
  if (!serialPortNum) {
    vscode.window.showWarningMessage(
      "未选择下载串口。请点击状态栏中的“COM: N/A”选择串口。",
    );
    return "";
  }
  ...
  const flashArguments = filesToFlash
    .map((item) => {
      // JSON 中的路径是相对于构建输出目录的,直接拼接即可
      const fullRelativePath = path
        .join(buildTargetFolder, item.file)
        .replace(/\\/g, "\\\\"); // Windows路径可能需要双反斜杠转义
...
async function updateConfiguration() {
  // 变为 async 函数,因为会调用 discoverSiFliSdks
  const config = vscode.workspace.getConfiguration("sifli-sdk-codekit");
  ...
  numThreads = config.get(
    "numThreads",
    os.cpus().length > 0 ? os.cpus().length : 8,
  );
  ...
  if (
    ...
    fs.existsSync(SIFLI_SDK_EXPORT_SCRIPT_PATH)
  ) {
    ...
    for (const sdkPath of installedPaths) {
      ...
      if (fs.existsSync(exportScript)) {
        ...
        vscode.window.showInformationMessage(
          `SiFli SDK export.ps1 脚本路径已自动设置为: ${exportScript}`,
        );
        foundValidPath = true;
        break;
      }
    ...
    if (!foundValidPath) {
      SIFLI_SDK_ROOT_PATH =
        ...
        vscode.workspace.workspaceFolders.length > 0
          ...
          : os.homedir();
      vscode.window.showWarningMessage(
        "SiFli SDK export.ps1 脚本路径未配置或无效，且未在已安装路径中找到有效 SDK。请在扩展设置中检查。",
      );
    }
  }
  ...
  console.log(
    `  SiFli SDK Export Script Path: ${SIFLI_SDK_EXPORT_SCRIPT_PATH}`,
  );
  ...
  for (const sdkPath of installedPaths) {
    ...
    if (fs.existsSync(exportScriptPath)) {
      ...
      try {
        ...
        console.error(
          `[SiFli Extension] 读取 SDK 版本信息失败 for ${sdkPath}: ${err.message}`,
        );
        sdkVersion = "读取失败";
      }
    } else {
...
function isSiFliProject() {
  if (
    ...
    console.log(
      "[SiFli Extension] No workspace folder open. Not a SiFli project.",
    );
    return false;
  }
  const workspaceRoot = vscode.workspace.workspaceFolders[0].uri.fsPath;
  const sconstructPathToCheck = path.join(
    workspaceRoot,
    SRC_SUBFOLDER,
    SCONSCRIPT_FILE,
  );
  ...
  console.log(
    `[SiFli Extension] Checking for SiFli project file: ${sconstructPathToCheck} - Found: ${fs.existsSync(sconstructPathToCheck)}`,
  );
  console.log(
    `[SiFli Extension] Checking for SDK environment (export.ps1): ${sifliSdkExportScriptPath} - Found: ${isSdkEnvironment}`,
  );
  return isProject;
}
...
async function getOrCreateSiFliTerminalAndCdProject() {
  console.log(
    `[SiFli Extension] Attempting to get or create terminal: ${TERMINAL_NAME}`,
  );
  let terminal = vscode.window.terminals.find((t) => t.name === TERMINAL_NAME);

  if (!terminal) {
    console.log(
      `[SiFli Extension] Terminal "${TERMINAL_NAME}" not found, creating a new one with specific profile.`,
    );
    ...
    if (
      ...
      if (
        ...
        console.log(
          `[SiFli Extension] Sent 'cd "${projectPath}"' to terminal.`,
        );
      } else {
        vscode.window.showWarningMessage(
          `SiFli: 无法找到 '${PROJECT_SUBFOLDER}' 文件夹。部分命令可能无法正常工作。`,
        );
        console.warn(
          `[SiFli Extension] Could not find '${PROJECT_SUBFOLDER}' folder at ${projectPath}.`,
        );
      }
    } else {
      vscode.window.showWarningMessage(
        "SiFli: 未打开工作区。命令可能无法在预期目录执行。",
      );
      console.warn("[SiFli Extension] No workspace folder open.");
    }
  } else {
    console.log(
      `[SiFli Extension] Terminal "${TERMINAL_NAME}" already exists.`,
    );
    if (
      vscode.workspace.workspaceFolders &&
      vscode.workspace.workspaceFolders.length > 0
      ...
      if (
        ...
        console.log(
          `[SiFli Extension] Resent 'cd "${projectPath}"' to existing terminal.`,
        );
      }
    }
  }
...
async function executeShellCommandInSiFliTerminal(commandLine, taskName) {
  // 移除 serialPortNumInput 参数
  const terminal = await getOrCreateSiFliTerminalAndCdProject();

  console.log(
    `[SiFli Extension] Sending command "${commandLine}" for task "${taskName}" to SF32 terminal.`,
  );
...
async function switchSdkVersion() {
  if (detectedSdkVersions.length === 0) {
    vscode.window.showInformationMessage(
      "未发现任何已安装的 SiFli SDK 版本。请通过“管理 SiFli SDK”功能进行安装。",
    );
    return;
  }
  ...
  if (selected) {
    if (!selected.isValid) {
      vscode.window.showWarningMessage(
        `选择的 SDK 路径无效：${selected.sdkPath}。请检查该路径下是否存在 'export.ps1'。`,
      );
      return;
    }
    ...
    if (selected.sdkPath === SIFLI_SDK_ROOT_PATH) {
      vscode.window.showInformationMessage(
        `SiFli SDK 已是当前版本：${selected.version}`,
      );
      return;
    }
    ...
    vscode.window.showInformationMessage(
      `SiFli SDK 已切换到版本: ${selected.version}`,
    );

    // 强制重新初始化终端以加载新的环境变量
    vscode.window.showInformationMessage(
      "SiFli SDK 环境已更新。正在重新加载终端...",
    );
...
async function getSerialPorts() {
  ...
  try {
    ...
    const tempScriptPath = path.join(
      os.tmpdir(),
      `get_serial_ports_${Date.now()}.ps1`,
    );
    fs.writeFileSync(tempScriptPath, powershellScriptContent, {
      encoding: "utf8",
    });
        ...
        exec(
          ...
          (error, stdout, stderr) => {
            ...
            try {
              ...
              console.warn(
                `[SiFli Extension] 无法删除临时 PowerShell 脚本文件 ${tempScriptPath}: ${cleanupError.message}`,
              );
            }

            if (error) {
              console.error(
                `[SiFli Extension] 执行 PowerShell 脚本失败: ${error.message}`,
              );
              return reject(error);
            }
    ...
    try {
      ...
      const portsArray = Array.isArray(psSerialPorts)
        ? psSerialPorts
        : psSerialPorts
          ? [psSerialPorts]
          : [];
      ...
      console.warn(
        `[SiFli Extension] 解析 PowerShell 串口信息失败 (可能没有可用串口或输出格式不符): ${parseError.message}`,
      );
      // 当没有串口时,stdout 可能为空或不是有效的 JSON,这里是预期行为
    }
  } catch (error) {
    vscode.window.showErrorMessage(
      `无法执行 PowerShell 命令获取串口列表。请确保 PowerShell 已正确安装并可访问。错误信息: ${error.message}`,
    );
    console.error(
      `[SiFli Extension] 获取串口失败 (PowerShell exec error): ${error.message}`,
    );
  }
...
async function selectSerialPort() {
  // 此函数不再是下载前的选择,而是通用的串口选择器
  try {
    const serialPorts = await getSerialPorts();

    if (serialPorts.length === 0) {
      vscode.window.showInformationMessage(
        "未检测到任何串行端口设备。请检查设备连接和驱动安装。",
      );
      ...
      vscode.window.showInformationMessage(
        `检测到单个串行端口设备,自动选择 COM 端口：${comPortNum}。`,
      );
...
async function scanDirectoryForBoards(directoryPath, boardMap, sourceType) {
  if (
    ...
    console.log(
      `[SiFli Extension] Board scan path does not exist or is not a directory: ${directoryPath}`,
    );
    return;
  }
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (
          fs.existsSync(hcpuPath) &&
          fs.lstatSync(hcpuPath).isDirectory() &&
          fs.existsSync(ptabJsonPath) &&
          fs.lstatSync(ptabJsonPath).isFile()
        ) {
          ...
          if (sourceType === "sdk" && boardMap.has(boardName)) {
            ...
            console.log(
              `[SiFli Extension] Skipping SDK board ${boardName} as higher priority board already exists.`,
            );
            continue;
          } else if (
            (sourceType === "custom" || sourceType === "project_local") &&
            boardMap.has(boardName)
          ) {
            // 如果当前是自定义或项目本地板子,且Map中已有同名板子,则覆盖
            console.log(
              `[SiFli Extension] Overwriting board ${boardName} with higher priority board from ${sourceType} path.`,
            );
          ...
          console.log(
            `[SiFli Extension] Found valid board: ${boardName} (Type: ${sourceType}, Path: ${boardFullPath})`,
          );
        } else {
          console.log(
            `[SiFli Extension] Not a valid board (missing hcpu or ptab.json): ${boardName} at ${boardFullPath}`,
          );
        }
      }
    }
    ...
    console.error(
      `[SiFli Extension] Failed to scan directory ${directoryPath} for boards: ${error.message}`,
    );
  }
}
...
async function discoverBoards() {
  ...
  if (!workspaceFolders || workspaceFolders.length === 0) {
    console.warn(
      "[SiFli Extension] No workspace folder open, cannot discover boards.",
    );
    return [];
  }
  const workspaceRoot = workspaceFolders[0].uri.fsPath;
  ...
  await scanDirectoryForBoards(
    projectLocalBoardsPath,
    discoveredBoardMap,
    "project_local",
  );
...
async function executeDownloadTask() {
  ...
  if (!selectedSerialPort) {
    // 将这里的警告改为信息提示,避免打扰用户
    vscode.window.showInformationMessage(
      '请先选择一个用于下载的串口。点击状态栏中的 "COM: N/A" 进行选择。',
    );
  ...
  const sftoolCommand = await getSftoolDownloadCommand(
    selectedBoardName,
    selectedSerialPort,
  );
  if (sftoolCommand) {
...
async function executeMenuconfigTask() {
  ...
  await executeShellCommandInSiFliTerminal(
    menuconfigCommand,
    MENUCONFIG_TASK_NAME,
  );
}
...
function executeCleanCommand() {
  ...
  const buildFolderPath = path.join(
    workspaceRoot,
    PROJECT_SUBFOLDER,
    getBuildTargetFolder(selectedBoardName),
  );

  console.log(
    `[SiFli Extension] Clean command: Checking for folder: ${buildFolderPath}`,
  );
  if (fs.existsSync(buildFolderPath)) {
    try {
      fs.rmSync(buildFolderPath, { recursive: true, force: true });
      vscode.window.showInformationMessage(
        `'${getBuildTargetFolder(selectedBoardName)}' 文件夹已成功删除。`,
      );
      console.log(
        `[SiFli Extension] Folder '${buildFolderPath}' deleted successfully.`,
      );
    } catch (error) {
      vscode.window.showErrorMessage(
        `删除 '${getBuildTargetFolder(selectedBoardName)}' 文件夹失败: ${error.message}`,
      );
      console.error(
        `[SiFli Extension] Clean failed for ${buildFolderPath}:`,
        error,
      );
    }
  } else {
    vscode.window.showInformationMessage(
      `'${getBuildTargetFolder(selectedBoardName)}' 文件夹不存在,无需删除。`,
    );
    console.log(
      `[SiFli Extension] Folder '${buildFolderPath}' not found, nothing to clean.`,
    );
  }
}
...
function initializeStatusBarItems(context) {
  ...
  sdkManageBtn = vscode.window.createStatusBarItem(
  ...
  // 新增：SDK 版本切换按钮
  currentSdkVersionStatusItem = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    101,
  );
  ...
  // 显示当前板卡的状态栏项
  currentBoardStatusItem = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    100,
  );
  ...
  // 显示当前串口的状态栏项
  currentSerialPortStatusItem = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    99,
  );
  ...
  compileBtn = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    98,
  );
  ...
  rebuildBtn = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    97,
  );
  ...
  cleanBtn = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    96,
  );
  ...
  downloadBtn = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    95,
  );
  ...
  menuconfigBtn = vscode.window.createStatusBarItem(
    vscode.StatusBarAlignment.Left,
    94,
  );
...
async function promptForInitialBoardSelection(context) {
  const hasRunInitialSetup = context.globalState.get(
    HAS_RUN_INITIAL_SETUP_KEY,
    false,
  );
  const config = vscode.workspace.getConfiguration("sifli-sdk-codekit");
  ...
  if (
    !hasRunInitialSetup ||
    !currentDefaultBoard ||
    !availableBoardsDetails.some((b) => b.name === currentDefaultBoard)
  ) {
    ...
    if (selected) {
      ...
      vscode.window.showInformationMessage(
        "已取消芯片模组选择。请稍后点击状态栏中的“SiFli Board: N/A”重新选择。",
      );
    }
    await context.globalState.update(HAS_RUN_INITIAL_SETUP_KEY, true);
...
async function isGitInstalled() {
  return new Promise((resolve) => {
    exec("git --version", (error) => {
      if (error) {
        console.error(
          `[SiFli Extension] Git is not installed or not in PATH: ${error.message}`,
        );
        resolve(false);
      } else {
        resolve(true);
    ...
    async (gitProgress) => {
      return new Promise((resolve, reject) => {
        ...
        gitOutputChannel.appendLine(
          `>>> 执行 Git 命令: git ${command} ${args.join(" ")} (工作目录: ${cwd})`,
        );
        ...
        gitProcess.stderr.on("data", (data) => {
          ...
          if (progressMatch && progressMatch[1]) {
            ...
            if (currentPercent !== lastKnownProgress) {
              gitProgress.report({
                increment: currentPercent - lastKnownProgress,
                message: `Git 进度: ${message}`,
              });
              lastKnownProgress = currentPercent;
            }
          } else if (
            message.includes("Cloning into") ||
        ...
        gitProcess.on("close", (code) => {
          if (code === 0) {
            ...
            gitProgress.report({
              message: `Git ${command} 完成。`,
              increment: 100 - lastKnownProgress,
            }); // 确保进度条走完
            resolve();
          } else {
...
async function fetchSiFliSdkReleases(source) {
  const url =
    source === "github"
      ? `${SIFLI_SDK_GITHUB_REPO_BASE}/releases`
      : `${SIFLI_SDK_GITEE_REPO_BASE}/releases`;
  ...
  try {
    ...
    console.error(
      `[SiFli SDK Manager] 获取发布版本失败 (${source}): ${error.message}`,
    );
    if (error.response) {
      console.error(
...
async function fetchSiFliSdkBranches(source) {
  const url =
    source === "github"
      ? `${SIFLI_SDK_GITHUB_REPO_BASE}/branches`
      : `${SIFLI_SDK_GITEE_REPO_BASE}/branches`;
  const headers = { "User-Agent": "VSCode-SiFli-Extension" };
  ...
  try {
    ...
    console.error(
      `[SiFli SDK Manager] 获取分支失败 (${source}): ${error.message}`,
    );
    if (error.response) {
      console.error(
        `Status: ${error.response.status}, Data:`,
...
function getSdkManagementWebviewContent(webview, extensionUri) {
  ...
  const scriptUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "WebView", "sdk_manager.js"),
  );
  const styleUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "WebView", "sdk_manager.css"),
  );
  const logoUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "images", "SiFli.png"),
  );
...
async function createSdkManagementWebview(context) {
  ...
  panel.webview.html = getSdkManagementWebviewContent(
    panel.webview,
      ...
      switch (message.command) {
        ...
        case "fetchReleases":
          try {
            ...
            vscode.window.showErrorMessage(
              `获取发布版本失败: ${error.message}`,
            );
            console.error(
              `[SiFli SDK Manager] 获取发布版本失败: ${error.message}`,
            );
          }
          return;
        case "fetchBranches": // 新增：处理获取分支的命令
          try {
            ...
            console.error(
              `[SiFli SDK Manager] 获取分支列表失败: ${error.message}`,
            );
          }
          return;
  ...
  await vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      const updateOverallProgress = (message, weight) => {
        ...
        gitOutputChannel.appendLine(
          `[SDK Installer] ${message} (${currentOverallProgress}%)`,
        );
      };
      ...
      try {
        ...
        if (type === "tag") {
          ...
          try {
            ...
            console.warn(
              `[SiFli SDK Manager] 无法获取最新版本信息或处理版本选择: ${error.message}`,
            );
          }
        }
        ...
        const repoUrl =
          source === "github"
            ? SIFLI_SDK_GITHUB_REPO_GIT
            : SIFLI_SDK_GITEE_REPO_GIT;
        const gitArgs = [
          "clone",
          "--recursive",
          "--progress",
        ...
        if (fs.existsSync(installPath)) {
          try {
            ...
            fs.rmSync(installPath, { recursive: true, force: true });
            gitOutputChannel.appendLine(
              `!!! 已尝试清理部分安装目录: ${installPath}`,
            );
          } catch (cleanupError) {
            gitOutputChannel.appendLine(
              `!!! 清理目录失败: ${cleanupError.message}`,
            );
            vscode.window.showWarningMessage(
              `安装失败，且无法完全清理目录: ${cleanupError.message}`,
            );
          }
        }
      } finally {
...
async function activate(context) {
  ...
  gitOutputChannel = vscode.window.createOutputChannel(
    "SiFli SDK Git Operations",
  );
  ...
  if (isSiFliProject()) {
    console.log(
      "[SiFli Extension] SiFli project detected. Activating full extension features.",
    );
    ...
    setTimeout(async () => {
      ...
      if (!selectedSerialPort) {
        vscode.window.showInformationMessage(
          '首次启动或串口未连接。请点击状态栏中的 "COM: N/A" 选择串口,以便进行下载操作。',
        );
      }
    ...
    context.subscriptions.push(
      ...
      vscode.commands.registerCommand(CMD_PREFIX + "manageSiFliSdk", () =>
        createSdkManagementWebview(context),
      ),
    ...
    console.log(
      "[SiFli Extension] Not a SiFli project. Extension features will not be activated.",
    );
  }
}
